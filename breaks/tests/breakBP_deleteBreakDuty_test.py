...
class TestBreakBP_deleteBreakDuty(unittest.TestCase):
    ...
    def test_whenPassedValidParams_returnsSuccessfulResponse(self):
        ...
        self.mocked_appGlobals.conn.cursor().fetchone.side_effect = [
            # First call returns ra table record data for the RA on duty
            (desiredOldID,),
            # Second call returns the dayID and monthID for the date string
        ...
        resp = self.server.post("/breaks/api/deleteBreakDuty",
                                json=dict(
                                    raName=desiredOldName,
        ...
        self.mocked_appGlobals.conn.cursor().execute.assert_called_with(
            """DELETE FROM break_duties
                        WHERE ra_id = %s
                        AND hall_id = %s
                        AND day_id = %s
                        AND month_id = %s""",
            (desiredOldID, self.user_hall_id, desiredDayID, desiredMonthID)
        )
    ...
    def test_whenPassedInvalidRA_returnsInvalidParamsResponse(self):
        ...
        #  met, then we expect to receive an "Unable to locate beak duty" result.
        ...
        self.assertEqual(resp.json, stdRet(-1, "Unable to find parameters in DB"))
    ...
    def test_whenPassedInvalidDay_returnsInvalidParamsResponse(self):
        ...
        #  an "Unable to locate beak duty" result.
